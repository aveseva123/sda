...
describe('кодирование конфигурации', () => {
  ...
  it('ползунки кодируются короткими ключами и разбираются обратно', () => {
    ...
    cfg.params = { ...financeDefaults, areaM2: 500, rentPerM2: 6.25, avgBudget: 40000 }
    const s = encodeConfig(cfg)
    expect(s).toBe('?area=500&rent=6.25&budget=40000')
  ...
  it('правки базовых строк оборудования: выключение, количество, цена', () => {
    const cfg = emptyConfig()
    cfg.equipment['cnc-1'] = { enabled: true, qty: 2, priceMin: 15000, priceMax: 20000 }
    cfg.equipment['saw'] = { enabled: false, qty: 1, priceMin: 6000, priceMax: 10000 }
    const s = encodeConfig(cfg)
    expect(s).toContain('eq=cnc-1|1|2|15000|20000,saw|0|1|6000|10000')
  ...
  it('правка, равная исходной строке, при разборе отбрасывается', () => {
    ...
    const cfg = decodeConfig(`?eq=${base.id}|1|${base.qty}|${base.priceMin}|${base.priceMax}`, equipment, team)
  ...
  it('неизвестные строки и битые значения игнорируются', () => {
    const cfg = decodeConfig('?eq=nope|0|1|1|1,cnc-1|1|abc|x|y&tm=ghost|0|1|1&c=unknown|5,payrollTaxPct|abc&sc=B', equipment, team)
    expect(cfg.equipment).toEqual({})
  ...
  it('свои строки с кириллицей и разделителями в названии переживают кодирование', () => {
    const cfg = emptyConfig()
    cfg.customEquipment.push({ name: 'Пресс | вакуумный, 2 стола', enabled: true, qty: 1, priceMin: 4000, priceMax: 7000, source: 'Китай', condition: 'новый' })
    cfg.customTeam.push({ role: 'Кладовщик, ночная смена', enabled: false, qty: 2, salary: 550 })
  ...
  it('кириллица в адресе остается однократно закодированной', () => {
    const cfg = emptyConfig()
    cfg.customTeam.push({ role: 'Кладовщик', enabled: true, qty: 1, salary: 600 })
    const s = encodeConfig(cfg)
    expect(s).toContain(encodeURIComponent('Кладовщик'))
  ...
  it('адрес остается совместимым с браузерным кодированием разделителей', () => {
    const cfg = emptyConfig()
    cfg.team['cnc'] = { enabled: true, qty: 2, salary: 950 }
    const s = encodeConfig(cfg)
...
describe('наложение конфигурации на данные', () => {
  ...
  it('правки применяются, свои строки получают идентификаторы x0, x1', () => {
    const cfg = emptyConfig()
    cfg.equipment['saw'] = { enabled: false, qty: 1, priceMin: 6000, priceMax: 10000 }
    cfg.customEquipment.push({ name: 'Вакуумный пресс', enabled: true, qty: 1, priceMin: 4000, priceMax: 7000, source: 'Китай', condition: 'новый' })
  ...
  it('штат: выключенная роль не идет в расчет, своя роль добавляется', () => {
    const cfg = emptyConfig()
    cfg.team['paint'] = { enabled: false, qty: 1, salary: 900 }
    cfg.customTeam.push({ role: 'Кладовщик', enabled: true, qty: 1, salary: 600 })
    const rows = effectiveTeam(team, cfg)
    expect(activeRows(rows).find((r) => r.id === 'paint')).toBeUndefined()
    expect(activeRows(rows).find((r) => r.role === 'Кладовщик')).toBeDefined()
  })
