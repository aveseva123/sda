...
import { phases, type Condition, type Phase, type Scenario, type Source } from '../data/equipment'
import { logistics, logisticsNote } from '../data/logistics'
import { equipmentCapex, equipmentRows } from '../model/finance'
import { CONDITIONS, SCENARIOS, SOURCES, type CustomEquipment } from '../model/config'
...
const scenarioOptions = SCENARIOS.map((s) => ({ id: s, title: s }))
...
export function Equipment({ config }: { config: ConfigApi }) {
  const { params, equipmentRows: rows, constants, updateEquipment, addEquipment, removeEquipment, resetEquipment, equipmentEdited, setParam } = config
  const scenario = params.scenario
  const [phase, setPhase] = useState<Phase>('m12')
  const [draft, setDraft] = useState<CustomEquipment>(emptyDraft)

  // В расчет идут включенные строки выбранного сценария до выбранной фазы включительно
  const counted = new Set(equipmentRows(rows.filter((r) => r.enabled), scenario, phase).map((r) => r.id))
  const capex = equipmentCapex(
    rows.filter((r) => counted.has(r.id)),
    constants,
  )
  ...
  return (
    <Section id="equipment" index={6} title="Оборудование" lead="Сценарий A — компактный старт, B — расширение по фазам. Цены за единицу без доставки. Строки можно выключать и править прямо здесь">
      <div className="flex flex-wrap items-center gap-3 mb-4 print-hide">
        <Segmented label="Сценарий" value={scenario} options={[{ id: 'A', title: 'Сценарий A' }, { id: 'B', title: 'Сценарий B' }]} onChange={(v: Scenario) => setParam('scenario', v)} />
        <Segmented label="Фаза" value={phase} options={phases} onChange={setPhase} />
        {equipmentEdited && <GhostButton onClick={resetEquipment}>Вернуть исходный список</GhostButton>}
      </div>
      <p className="text-sm text-muted mb-4">
        В итог входят включенные строки сценария {scenario} до фазы «{phaseTitle[phase]}» включительно: {counted.size} из {rows.length}. Остальные показаны бледнее
        {equipmentEdited && (
          <>
            {' '}
      ...
      <Card className="p-0! sm:p-0!">
        ...
        <div className="overflow-x-auto">
          <table className="tbl min-w-[1200px]">
            ...
            <tbody>
              {rows.map((r) => {
                const inCount = counted.has(r.id)
                const cls = !r.enabled ? 'row-off' : !inCount ? 'row-dim' : ''
                return (
                  <tr key={r.id} className={cls}>
                    ...
                    <td>
                      <SelectInput value={r.phase} options={phaseOptions} label={`${r.name}: фаза`} onChange={(v: Phase) => updateEquipment(r.id, { phase: v })} />
                    </td>
                    <td>
                      <SelectInput value={r.scenario} options={scenarioOptions} label={`${r.name}: сценарий`} onChange={(v: Scenario) => updateEquipment(r.id, { scenario: v })} />
                    </td>
            ...
            <tfoot>
              <tr>
                ...
                <td className="num">{rows.filter((r) => counted.has(r.id)).reduce((s, r) => s + r.qty, 0)}</td>
                <td className="num" colSpan={2}>
                  {usdRange(capex.equipment.min, capex.equipment.max)}
                </td>
                <td colSpan={6} className="text-muted font-normal">
        ...
        <form onSubmit={add} className="print-hide border-t border-white/10 p-4 sm:p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
            <TextInput value={draft.name} onChange={(v) => setDraft({ ...draft, name: v })} label="Станок" required className="col-span-2" />
