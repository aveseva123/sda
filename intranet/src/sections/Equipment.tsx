...
export function Equipment({ config, edit }: { config: ConfigApi; edit: boolean }) {
  ...
  const hiddenCount = rows.length - active.length
    ...
    <Section id="equipment" index={6} title="Оборудование" lead="Стартовый набор для корпусной мебели полного цикла. Цены за единицу без доставки. Металл не делаем: каркасы и нержавейка — субподряд">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <p className="text-sm text-muted">
          {edit ? (
            ...
            <>
              {active.length} позиций, итог с доставкой {usdRange(capex.total.min, capex.total.max)}
              {hiddenCount > 0 && `. ${hiddenCount} строк выключено и не показано`}
            </>
          )}
          {equipmentEdited && (
            <>
      ...
      <Card className="p-0! sm:p-0!">
        ...
        <div className="overflow-x-auto">
          <table className={`tbl ${edit ? 'min-w-[1000px]' : 'min-w-[820px]'}`}>
            <thead>
              <tr>
                      ...
                      <span>
                        ...
                        {r.model && (
                          <span className="block text-xs text-muted">
                            {r.link ? (
                              <a href={r.link} target="_blank" rel="noreferrer" className="underline decoration-white/30 hover:text-amber">
                                {r.model}
                              </a>
                            ) : (
                              r.model
                            )}
                            {r.priceDate && ` · цена ${r.priceDate}`}
                          </span>
                        )}
                        {edit && !r.enabled && <span className="block text-xs text-muted">Не входит в расчет</span>}
                      </span>
                    </div>
                  </td>
                  <td className="num">
            ...
            <tfoot>
              <tr>
                ...
                <td className="num" colSpan={edit ? 2 : 1}>
                  {usdRange(capex.equipment.min, capex.equipment.max)}
                </td>
                <td colSpan={edit ? 4 : 3} className="text-muted font-normal">
              ...
              <tr>
                ...
                <td />
                <td className="num text-amber" colSpan={edit ? 2 : 1}>
